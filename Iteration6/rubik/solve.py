...
# Assign the token length for the hashes
TOKEN_LENGTH = 8

def _solve(parms):
    """ Return rotations needed to solve input cube """
    result = {}
    rotations = ''
    ...
    rotations += solveCube.get('rotations')
    solveCube = _solveMiddleLayer(solveCube)
    rotations += solveCube.get('rotations')
    solveCube = _solveUpCross(solveCube)
    rotations += solveCube.get('rotations')
    solveCube = _solveUpFace(solveCube)
    rotations += solveCube.get('rotations')
    solveCube = _solveUpCorners(solveCube)
    rotations += solveCube.get('rotations')
    solveCube = _solveUpMiddlePieces(solveCube)
    rotations += solveCube.get('rotations')
    
    # Hash the result
    itemToTokenize = parms.get('cube') + rotations
    sha256Hash = hashlib.sha256()
    sha256Hash.update(itemToTokenize.encode())
    fullToken = sha256Hash.hexdigest()
    randomNumber = random.randint(0, len(fullToken) - TOKEN_LENGTH)
    
    # Put the solution in result and return it
    result['rotations'] = rotations
    result['status'] = 'ok' 
    result['token'] = fullToken[randomNumber:(randomNumber + TOKEN_LENGTH)]
...
def _solveMiddleLayer(parms):
    """ Method to solve the middle layer of the cube """
    solveCube = {}
    solveCube['cube'] = parms.get('cube')
    rotations = ''
    searchDict = {}
    ...
    # For each middle layer edge check if it is already solved and if not solve it
    searchDict['cube'] = solveCube.get('cube')
    searchDict['colors'] = {solveCube.get('cube')[F11], solveCube.get('cube')[L11]}
    if _findMiddleLayerPiece(searchDict) != F10:
        solveCube = _solveFrontLeftMiddlePiece(solveCube)
        rotations += solveCube.get('rotations')
        
    searchDict['cube'] = solveCube.get('cube')
    searchDict['colors'] = {solveCube.get('cube')[F11], solveCube.get('cube')[R11]}
    if _findMiddleLayerPiece(searchDict) != F12:
        solveCube = _solveFrontRightMiddlePiece(solveCube)
        rotations += solveCube.get('rotations')
    
    searchDict['cube'] = solveCube.get('cube')
    searchDict['colors'] = {solveCube.get('cube')[B11], solveCube.get('cube')[L11]}
    if _findMiddleLayerPiece(searchDict) != B12:
        solveCube = _solveBackLeftMiddlePiece(solveCube)
        rotations += solveCube.get('rotations')
    
    searchDict['cube'] = solveCube.get('cube')
    searchDict['colors'] = {solveCube.get('cube')[B11], solveCube.get('cube')[R11]}
    if _findMiddleLayerPiece(searchDict) != B10:
        solveCube = _solveBackRightMiddlePiece(solveCube)
        rotations += solveCube.get('rotations')
    
    # Return the rotations to solve the middle layer and the resulting cube
    result['cube'] = solveCube.get('cube')
    result['rotations'] = rotations
    return result

def _solveUpCross(parms):
    """ Method to solve the up surface cross of the cube """
    solveCube = parms.get('cube')
    rotations = ''
    output = {} 
    
    # Check what configuration the up cross is in and get the rotations to solve that configuration
    if solveCube[U21] != solveCube[U11] or solveCube[U12] != solveCube[U11] or solveCube[U10] != solveCube[U11] or solveCube[U01] != solveCube[U11]:
        if solveCube[U21] != solveCube[U11] and solveCube[U12] != solveCube[U11] and solveCube[U10] != solveCube[U11] and solveCube[U01] != solveCube[U11]:
            rotations = 'FRUrufuuFRUruRUruf'
        elif solveCube[U21] == solveCube[U11] and solveCube[U12] == solveCube[U11]: rotations = 'uuFRUruRUruf'
        elif solveCube[U21] == solveCube[U11] and solveCube[U10] == solveCube[U11]: rotations = 'UFRUruRUruf'
        elif solveCube[U01] == solveCube[U11] and solveCube[U10] == solveCube[U11]: rotations = 'FRUruRUruf'
        elif solveCube[U01] == solveCube[U11] and solveCube[U12] == solveCube[U11]: rotations = 'uFRUruRUruf'
        elif solveCube[U01] == solveCube[U11] and solveCube[U21] == solveCube[U11]: rotations = 'uFRUruf'
        elif solveCube[U10] == solveCube[U11] and solveCube[U12] == solveCube[U11]: rotations = 'FRUruf'
        rotatedCube = _rotatePiece(rotations, solveCube)
    else: rotatedCube = solveCube  
    
    # Return the rotations to get correct up surface cross and the cube after rotations
    output['rotations'] = rotations
    output['cube'] = rotatedCube
    return output

def _solveUpFace(parms):
    """ Method to solve the up face of the cube """
    solveCube = parms.get('cube')
    rotations = ''
    allRotations = ''
    output = {} 
    
    # Check what configuration the up face is in and get the rotations to solve that configuration
    while (solveCube[U00] != solveCube[U11]) or (solveCube[U02] != solveCube[U11]) or (solveCube[U20] != solveCube[U11]) or (solveCube[U22] != solveCube[U11]):
        if solveCube[U00] == solveCube[U11] and solveCube[U02] != solveCube[U11] and solveCube[U20] != solveCube[U11] and solveCube[U22] != solveCube[U11]:
            rotations = 'uRUrURUUr'
        elif solveCube[U00] != solveCube[U11] and solveCube[U02] == solveCube[U11] and solveCube[U20] != solveCube[U11] and solveCube[U22] != solveCube[U11]:
            rotations = 'UURUrURUUr'
        elif solveCube[U00] != solveCube[U11] and solveCube[U02] != solveCube[U11] and solveCube[U20] == solveCube[U11] and solveCube[U22] != solveCube[U11]:
            rotations = 'RUrURUUr'
        elif solveCube[U00] != solveCube[U11] and solveCube[U02] != solveCube[U11] and solveCube[U20] != solveCube[U11] and solveCube[U22] == solveCube[U11]:
            rotations = 'URUrURUUr'
        else:
            if solveCube[L02] == solveCube[U11]: rotations += 'RUrURUUr'
            elif solveCube[F02] == solveCube[U11]: rotations += 'URUrURUUr'
            elif solveCube[B02] == solveCube[U11]: rotations += 'uRUrURUUr'
            else: rotations += 'UURUrURUUr'
        allRotations += rotations
        solveCube = _rotatePiece(rotations, solveCube) 
        rotations = ''
        
    # Return the rotations to get correct up face and the cube after rotations
    output['rotations'] = allRotations
    output['cube'] = solveCube
    return output

def _solveUpCorners(parms):
    """ Method to solve the up corners of the cube """
    solveCube = parms.get('cube')
    rotations = ''
    allRotations = ''
    output = {} 
    
    # Check what configuration the up corners are in and get the rotations to solve that configuration
    if solveCube[F00] != solveCube[F02] or solveCube[R00] != solveCube[R02] or solveCube[B00] != solveCube[B02] or solveCube[L00] != solveCube[L02]:
        if solveCube[F00] != solveCube[F02] and solveCube[R00] != solveCube[R02] and solveCube[B00] != solveCube[B02] and solveCube[L00] != solveCube[L02]:
            rotations = 'lURuLUUrURUUr'
            allRotations += rotations
            solveCube = _rotatePiece(rotations, solveCube)
        if solveCube[F00] == solveCube[F02] and solveCube[F00] == solveCube[F11]: rotations = 'fUBuFUUbUBUUb'
        elif solveCube[R00] == solveCube[R02] and solveCube[R00] == solveCube[F11]: rotations = 'UfUBuFUUbUBUUb'
        elif solveCube[B00] == solveCube[B02] and solveCube[B00] == solveCube[F11]: rotations = 'UUfUBuFUUbUBUUb'
        elif solveCube[L00] == solveCube[L02] and solveCube[L00] == solveCube[F11]: rotations = 'ufUBuFUUbUBUUb'
        elif solveCube[F00] == solveCube[F02] and solveCube[F00] == solveCube[R11]: rotations = 'urULuRUUlULUUl'
        elif solveCube[R00] == solveCube[R02] and solveCube[R00] == solveCube[R11]: rotations = 'rULuRUUlULUUl'
        elif solveCube[B00] == solveCube[B02] and solveCube[B00] == solveCube[R11]: rotations = 'UrULuRUUlULUUl'
        elif solveCube[L00] == solveCube[L02] and solveCube[L00] == solveCube[R11]: rotations = 'UUrULuRUUlULUUl'
        elif solveCube[F00] == solveCube[F02] and solveCube[F00] == solveCube[B11]: rotations = 'UUbUFuBUUfUFUUf'
        elif solveCube[R00] == solveCube[R02] and solveCube[R00] == solveCube[B11]: rotations = 'ubUFuBUUfUFUUf'
        elif solveCube[B00] == solveCube[B02] and solveCube[B00] == solveCube[B11]: rotations = 'bUFuBUUfUFUUf'
        elif solveCube[L00] == solveCube[L02] and solveCube[L00] == solveCube[B11]: rotations = 'UbUFuBUUfUFUUf'
        elif solveCube[F00] == solveCube[F02] and solveCube[F00] == solveCube[L11]: rotations = 'UlURuLUUrURUUr'
        elif solveCube[R00] == solveCube[R02] and solveCube[R00] == solveCube[L11]: rotations = 'UUlURuLUUrURUUr'
        elif solveCube[B00] == solveCube[B02] and solveCube[B00] == solveCube[L11]: rotations = 'ulURuLUUrURUUr'
        elif solveCube[L00] == solveCube[L02] and solveCube[L00] == solveCube[L11]: rotations = 'lURuLUUrURUUr'
        allRotations += rotations
        solveCube = _rotatePiece(rotations, solveCube)
    elif solveCube[F00] != solveCube[F11]: # If corners are in correct position, check that the corners are on the correct side of the cube
        while solveCube[F00] != solveCube[F11]:
            rotations = 'U'
            allRotations += rotations
            solveCube = _rotatePiece(rotations, solveCube)
            
    # Return the rotations to get correct up corners and the cube after rotations
    output['rotations'] = allRotations
    output['cube'] = solveCube
    return output

def _solveUpMiddlePieces(parms):
    """ Method to solve the up middle pieces of the cube """
    solveCube = parms.get('cube')
    rotations = ''
    allRotations = ''
    output = {} 
    
    # Check what configuration the up middle pieces are in and get the rotations to solve that configuration
    if solveCube[F01] != solveCube[F11] or solveCube[R01] != solveCube[R11] or solveCube[B01] != solveCube[B11] or solveCube[L01] != solveCube[L11]:
        if solveCube[F01] != solveCube[F11] and solveCube[R01] != solveCube[R11] and solveCube[B01] != solveCube[B11] and solveCube[L01] != solveCube[L11]:
            rotations = 'FFUrLFFlRUFF'
            allRotations += rotations
            solveCube = _rotatePiece(rotations, solveCube)
        if solveCube[F01] == solveCube[F11] and solveCube[R01] == solveCube[L11]: rotations = 'BBUlRBBrLUBB'
        elif solveCube[F01] == solveCube[F11] and solveCube[R01] == solveCube[B11]: rotations = 'BBUlRBBrLUUlRBBrLUBB'
        elif solveCube[R01] == solveCube[R11] and solveCube[B01] == solveCube[F11]: rotations = 'LLUfBLLbFULL'
        elif solveCube[R01] == solveCube[R11] and solveCube[B01] == solveCube[L11]: rotations = 'LLUfBLLbFUUfBLLbFULL'
        elif solveCube[B01] == solveCube[B11] and solveCube[L01] == solveCube[R11]: rotations = 'FFUrLFFlRUFF'
        elif solveCube[B01] == solveCube[B11] and solveCube[L01] == solveCube[F11]: rotations = 'FFUrLFFlRUUrLFFlRUFF'
        elif solveCube[L01] == solveCube[L11] and solveCube[F01] == solveCube[B11]: rotations = 'RRUbFRRfBURR'
        elif solveCube[L01] == solveCube[L11] and solveCube[F01] == solveCube[R11]: rotations = 'RRUbFRRfBUUbFRRfBURR'
        allRotations += rotations
        solveCube = _rotatePiece(rotations, solveCube)
    
    # Return the rotations to get correct up middle pieces and the cube after rotations
    output['rotations'] = allRotations
    output['cube'] = solveCube
    return output
...
def _solveFrontLeftMiddlePiece(parms):
    """ Finds the correct middle piece for the front left middle layer piece and the rotations to put it in place """
    rotations = ''
    inputCube = parms.get('cube')
    output = {}
    searchDict = {}
    searchDict['cube'] = inputCube
    searchDict['colors'] = {inputCube[F11], inputCube[L11]}
    
    # Find correct middle piece and corresponding rotation for that location
    if _findMiddleLayerPiece(searchDict) == F01: rotations = 'ulULUFuf'
    elif _findMiddleLayerPiece(searchDict) == F12: rotations = 'RurufUFFufulUL'
    elif _findMiddleLayerPiece(searchDict) == R01: rotations = 'lULUFuf'
    elif _findMiddleLayerPiece(searchDict) == R10: rotations = 'RurufUFUlULUFuf'
    elif _findMiddleLayerPiece(searchDict) == R12: rotations = 'BuburURUFufulUL'
    elif _findMiddleLayerPiece(searchDict) == B01: rotations = 'UlULUFuf'
    elif _findMiddleLayerPiece(searchDict) == B10: rotations = 'BuburURuulULUFuf'
    elif _findMiddleLayerPiece(searchDict) == B12: rotations = 'LulubUBUUFufulUL'
    elif _findMiddleLayerPiece(searchDict) == L01: rotations = 'uulULUFuf'
    elif _findMiddleLayerPiece(searchDict) == L10: rotations = 'LulubUBulULUFuf'
    elif _findMiddleLayerPiece(searchDict) == L12: rotations = 'lULUFufUlULUFuf'
    elif _findMiddleLayerPiece(searchDict) == U01: rotations = 'FufulUL'
    elif _findMiddleLayerPiece(searchDict) == U10: rotations = 'UFufulUL'
    elif _findMiddleLayerPiece(searchDict) == U12: rotations = 'uFufulUL'
    elif _findMiddleLayerPiece(searchDict) == U21: rotations = 'uuFufulUL'
    rotatedCube = _rotatePiece(rotations, inputCube)    
    
    # Return the rotations to get solve the front left middle layer piece and the resulting cube
    output['rotations'] = rotations
    output['cube'] = rotatedCube
    return output

def _solveFrontRightMiddlePiece(parms):
    """ Finds the correct middle piece for the front right middle layer piece and the rotations to put it in place """
    rotations = ''
    inputCube = parms.get('cube')
    output = {}
    searchDict = {}
    searchDict['cube'] = inputCube
    searchDict['colors'] = {inputCube[F11], inputCube[R11]}
    
    # Find correct middle piece and corresponding rotation for that location
    if _findMiddleLayerPiece(searchDict) == F01: rotations = 'URurufUF'
    elif _findMiddleLayerPiece(searchDict) == R01: rotations = 'UURurufUF'
    elif _findMiddleLayerPiece(searchDict) == R10: rotations = 'RurufUFuRurufUF'
    elif _findMiddleLayerPiece(searchDict) == R12: rotations = 'BuburURUfUFURur'
    elif _findMiddleLayerPiece(searchDict) == B01: rotations = 'uRurufUF'
    elif _findMiddleLayerPiece(searchDict) == B10: rotations = 'BuburURRurufUF'
    elif _findMiddleLayerPiece(searchDict) == B12: rotations = 'bUBULulUURurufUF'
    elif _findMiddleLayerPiece(searchDict) == L01: rotations = 'RurufUF'
    elif _findMiddleLayerPiece(searchDict) == L10: rotations = 'LulubUBURurufUF'
    elif _findMiddleLayerPiece(searchDict) == U01: rotations = 'fUFURur'
    elif _findMiddleLayerPiece(searchDict) == U10: rotations = 'UfUFURur'
    elif _findMiddleLayerPiece(searchDict) == U12: rotations = 'ufUFURur'
    elif _findMiddleLayerPiece(searchDict) == U21: rotations = 'uufUFURur'
    rotatedCube = _rotatePiece(rotations, inputCube)    
    
    # Return the rotations to get solve the front right middle layer piece and the resulting cube
    output['rotations'] = rotations
    output['cube'] = rotatedCube
    return output

def _solveBackLeftMiddlePiece(parms):
    """ Finds the correct middle piece for the back left middle layer piece and the rotations to put it in place """
    rotations = ''
    inputCube = parms.get('cube')
    output = {}
    searchDict = {}
    searchDict['cube'] = inputCube
    searchDict['colors'] = {inputCube[B11], inputCube[L11]}
    
    # Find correct middle piece and corresponding rotation for that location
    if _findMiddleLayerPiece(searchDict) == F01: rotations = 'uLulubUB'
    elif _findMiddleLayerPiece(searchDict) == R01: rotations = 'LulubUB' 
    elif _findMiddleLayerPiece(searchDict) == R12: rotations = 'BuburURubUBULul' 
    elif _findMiddleLayerPiece(searchDict) == B01: rotations = 'ULulubUB'   
    elif _findMiddleLayerPiece(searchDict) == B10: rotations = 'BuburURuuLulubUB'   
    elif _findMiddleLayerPiece(searchDict) == L01: rotations = 'uuLulubUB'   
    elif _findMiddleLayerPiece(searchDict) == L10: rotations = 'LulubUBuLulubUB'  
    elif _findMiddleLayerPiece(searchDict) == U01: rotations = 'uubUBULul'   
    elif _findMiddleLayerPiece(searchDict) == U10: rotations = 'ubUBULul'
    elif _findMiddleLayerPiece(searchDict) == U12: rotations = 'UbUBULul'  
    elif _findMiddleLayerPiece(searchDict) == U21: rotations = 'bUBULul'   
    rotatedCube = _rotatePiece(rotations, inputCube)    
    
    # Return the rotations to get solve the back left middle layer piece and the resulting cube
    output['rotations'] = rotations
    output['cube'] = rotatedCube
    return output

def _solveBackRightMiddlePiece(parms):
    """ Finds the correct middle piece for the back left middle layer piece and the rotations to put it in place """
    rotations = ''
    inputCube = parms.get('cube')
    output = {}
    searchDict = {}
    searchDict['cube'] = inputCube
    searchDict['colors'] = {inputCube[B11], inputCube[R11]}
    
    # Find correct middle piece and corresponding rotation for that location
    if _findMiddleLayerPiece(searchDict) == F01: rotations = 'UrURUBub' 
    elif _findMiddleLayerPiece(searchDict) == R01: rotations = 'uurURUBub' 
    elif _findMiddleLayerPiece(searchDict) == R12: rotations = 'rURUBubUrURUBub' 
    elif _findMiddleLayerPiece(searchDict) == B01: rotations = 'urURUBub' 
    elif _findMiddleLayerPiece(searchDict) == L01: rotations = 'rURUBub' 
    elif _findMiddleLayerPiece(searchDict) == U01: rotations = 'UUBuburUR' 
    elif _findMiddleLayerPiece(searchDict) == U10: rotations = 'uBuburUR' 
    elif _findMiddleLayerPiece(searchDict) == U12: rotations = 'UBuburUR' 
    elif _findMiddleLayerPiece(searchDict) == U21: rotations = 'BuburUR' 
    rotatedCube = _rotatePiece(rotations, inputCube)    
...
def _findMiddleLayerPiece(parms):
    """ Method that finds a middle layer piece and returns the location of the front or back color of that piece """
    searchCube = parms.get('cube')
    searchColors = parms.get('colors')
    location = None
    
    # Set of all the middle pieces of the cube that could go in the middle layer
    middles = {frozenset([F01, U21]), frozenset([F10, L12]), frozenset([F12, R10]), frozenset([L01, U10]),
               frozenset([R01, U12]), frozenset([B10, R12]), frozenset([B01, U01]), frozenset([B12, L10])}
    
    # Go through the set of possible middle pieces and find one with the colors being searched for
    for middle in middles: 
        currentColors = set()
        for face in middle:
            currentColors.add(searchCube[face])
            if searchCube[face] == searchCube[F11]:
                location = face
            elif searchCube[face] == searchCube[B11]:
                location = face
        if searchColors == currentColors:
            # Return the location of the front or back color of the piece
            return location
